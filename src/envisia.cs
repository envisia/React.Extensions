...
internal struct RustBuffer {
    ...
    public static RustBuffer Alloc(int size) {
        return _UniffiHelpers.RustCall((ref RustCallStatus status) => {
            var buffer = _UniFFILib.ffi_envisia_a9e8_rustbuffer_alloc(size, ref status);
    ...
    public static void Free(RustBuffer buffer) {
        _UniffiHelpers.RustCall((ref RustCallStatus status) => {
            _UniFFILib.ffi_envisia_a9e8_rustbuffer_free(buffer, ref status);
        });
    }
...
static class _UniFFILib {
    static _UniFFILib() {
        ...
        FfiConverterTypeTaskResult.INSTANCE.Register();
        }

    [DllImport("envisia_wasm")]
    public static extern void ffi_envisia_a9e8_ComponentRenderer_object_free(IntPtr @ptr,
    ref RustCallStatus _uniffi_out_err
    );

    [DllImport("envisia_wasm")]
    public static extern ComponentRendererSafeHandle envisia_a9e8_ComponentRenderer_new(
    ref RustCallStatus _uniffi_out_err
    );

    [DllImport("envisia_wasm")]
    public static extern void envisia_a9e8_ComponentRenderer_render_component(ComponentRendererSafeHandle @ptr,ulong @result,RustBuffer @file,RustBuffer @name,RustBuffer @values,
    ref RustCallStatus _uniffi_out_err
    );

    [DllImport("envisia_wasm")]
    public static extern void ffi_envisia_a9e8_TaskResult_init_callback(ForeignCallback @callbackStub,
    ref RustCallStatus _uniffi_out_err
    );

    [DllImport("envisia_wasm")]
    public static extern RustBuffer ffi_envisia_a9e8_rustbuffer_alloc(int @size,
    ref RustCallStatus _uniffi_out_err
    );

    [DllImport("envisia_wasm")]
    public static extern RustBuffer ffi_envisia_a9e8_rustbuffer_from_bytes(ForeignBytes @bytes,
    ref RustCallStatus _uniffi_out_err
    );

    [DllImport("envisia_wasm")]
    public static extern void ffi_envisia_a9e8_rustbuffer_free(RustBuffer @buf,
    ref RustCallStatus _uniffi_out_err
    );

    [DllImport("envisia_wasm")]
    public static extern RustBuffer ffi_envisia_a9e8_rustbuffer_reserve(RustBuffer @buf,int @additional,
    ref RustCallStatus _uniffi_out_err
    );
...
public interface IComponentRenderer {
    ...
    void RenderComponent(TaskResult @result, String @file, String @name, String @values);
    
}

public class ComponentRendererSafeHandle: FFISafeHandle {
    public ComponentRendererSafeHandle(): base() {
    }
    public ComponentRendererSafeHandle(IntPtr pointer): base(pointer) {
    }
    override protected bool ReleaseHandle() {
        _UniffiHelpers.RustCall((ref RustCallStatus status) => {
            _UniFFILib.ffi_envisia_a9e8_ComponentRenderer_object_free(this.handle, ref status);
        });
        return true;
    }
}
public class ComponentRenderer: FFIObject<ComponentRendererSafeHandle>, IComponentRenderer {
    public ComponentRenderer(ComponentRendererSafeHandle pointer): base(pointer) {}
    public ComponentRenderer() :
        this(
    _UniffiHelpers.RustCall( (ref RustCallStatus _status) =>
    _UniFFILib.envisia_a9e8_ComponentRenderer_new( ref _status)
)) {}

    
    public void RenderComponent(TaskResult @result, String @file, String @name, String @values) {
    _UniffiHelpers.RustCall( (ref RustCallStatus _status) =>
    _UniFFILib.envisia_a9e8_ComponentRenderer_render_component(this.GetHandle(), FfiConverterTypeTaskResult.INSTANCE.Lower(@result), FfiConverterString.INSTANCE.Lower(@file), FfiConverterString.INSTANCE.Lower(@name), FfiConverterString.INSTANCE.Lower(@values), ref _status)
);
    }
...
class FfiConverterTypeComponentRenderer: FfiConverter<ComponentRenderer, ComponentRendererSafeHandle> {
    public static FfiConverterTypeComponentRenderer INSTANCE = new FfiConverterTypeComponentRenderer();

    public override ComponentRendererSafeHandle Lower(ComponentRenderer value) {
        return value.GetHandle();
    }

    public override ComponentRenderer Lift(ComponentRendererSafeHandle value) {
        return new ComponentRenderer(value);
    }

    public override ComponentRenderer Read(BigEndianStream stream) {
        return Lift(new ComponentRendererSafeHandle(new IntPtr(stream.ReadLong())));
    }

    public override int AllocationSize(ComponentRenderer value) {
        return 8;
    }

    public override void Write(ComponentRenderer value, BigEndianStream stream) {
...
class ForeignCallbackTypeTaskResult {
    ...
    public static ForeignCallback INSTANCE = (ulong handle, int method, RustBuffer args, ref RustBuffer outBuf) => {
        var cb = FfiConverterTypeTaskResult.INSTANCE.Lift(handle);
        switch (method) {
            case 0: {
                // 0 means Rust is done with the callback, and the callback
                // can be dropped by the foreign language.
                FfiConverterTypeTaskResult.INSTANCE.Drop(handle);
                // No return value.
    ...
    static RustBuffer InvokeSuccess(TaskResult callback, RustBuffer args) {
        try {
            var stream = args.AsStream();
            callback.Success(FfiConverterString.INSTANCE.Read(stream));
    ...
    static RustBuffer InvokeError(TaskResult callback, RustBuffer args) {
        try {
            var stream = args.AsStream();
            callback.Error(FfiConverterString.INSTANCE.Read(stream));
...
class FfiConverterTypeTaskResult: FfiConverterCallbackInterface<TaskResult> {
    public static FfiConverterTypeTaskResult INSTANCE = new FfiConverterTypeTaskResult();

    public override void Register() {
        _UniffiHelpers.RustCall((ref RustCallStatus status) => {
            _UniFFILib.ffi_envisia_a9e8_TaskResult_init_callback(ForeignCallbackTypeTaskResult.INSTANCE, ref status);
        });
    }
}
#pragma warning restore 8625

public static class EnvisiaMethods {
}
