...
using Envisia.React.Engine.V8.Resources;
...
public sealed class V8JsEngine : JsEngineBase
{
    ...
    /// </summary>
    public V8JsEngine() : this(new V8Settings())
    {
    }
    ...
    #region Envisia JsEngineBase Overwrites

    public override T Evaluate<T>(string expression)
    {
        VerifyNotDisposed();
        ...
        if (expression == null)
        {
            throw new ArgumentNullException(
                nameof(expression),
                string.Format(CoreStrings.Common_ArgumentIsNull, nameof(expression))
            );
        }
        ...
        if (string.IsNullOrWhiteSpace(expression))
        {
            throw new ArgumentException(
                string.Format(CoreStrings.Common_ArgumentIsEmpty, nameof(expression)),
                nameof(expression)
            );
        }

        Type returnValueType = typeof(T);
        if (!ValidationHelpers.IsSupportedType(returnValueType) && returnValueType != typeof(Stream))
        {
            throw new ArgumentException(
                string.Format(CoreStrings.Usage_ReturnValueTypeNotSupported, returnValueType.FullName),
                nameof(T)
            );
        }
    ...
    public override T Evaluate<T>(string expression, string documentName)
    {
        VerifyNotDisposed();
        ...
        {
            throw new ArgumentException(
                string.Format(CoreStrings.Usage_InvalidDocumentNameFormat, documentName),
                nameof(documentName)
            );
        }
